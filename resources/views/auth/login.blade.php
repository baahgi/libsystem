                ...
                <div class="flex flex-col break-words bg-white border border-2 rounded shadow-md">
                    ...
                    <form class="w-full p-6" method="POST" action="{{ route('login') }}">
                        ...
                        <div class="flex flex-wrap mb-6">
                            <label for="login" class="block mb-2 text-sm font-bold text-gray-700">
                                {{ __('E-Mail Address or ID') }}:
                            </label>

                            <input id="login" type="text" class="form-input w-full {{ $errors->has('email') || $errors->has('student_id') ? 'border-red-500' : '' }}" name="login" value="{{ old('email') ?: old('student_id') }}" required autocomplete="login" autofocus>

                            @if($errors->has('email') || $errors->has('student_id'))
                                <p class="mt-4 text-xs italic text-red-500">
                                    {{ $errors->first('email' ?: $errors->first('studnet_id')) }}
                                </p>
                            @endif
                        </div>
